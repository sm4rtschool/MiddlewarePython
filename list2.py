    ...
    def start_reading(reader_id, root):
        ...
        def clear_treeview(reader_id):
            ...
            for tag in unique_tags:
                ...
                try:
                    data = {'rfid_id': cleaned_tag, 'librarian_id': librarian_id}
                    print(f"Sending data: {data}")
                    response = requests.post(location_update_url, data=data, headers=headers)
                    print(response.text)
                    response.raise_for_status()
                    result = response.json()
            ...
            try:
                while reader_status[reader_id]['reading_enabled']:
                    for response in responses:
                        ...
                        # Print librarian_id
                        print(
                            f"Librarian ID for reader {reader_id}: {reader_status[reader_id].get('librarian_id', 'N/A')}")
            except Exception as e:
    ...
    if tag_readers:
        ...
        for reader in tag_readers:
            if isinstance(reader, dict):
                ...
                connect_button = tk.Button(frame, text="Connect", width=12,
                                           command=lambda r=reader_id: toggle_connection(r))
                connect_button.grid(row=12, column=0, padx=10, pady=10, sticky='ew')

                mode_button = tk.Button(frame, text="Read Mode", width=12, state=tk.DISABLED,
                                        command=lambda r=reader_id: toggle_mode(r))
                ...
                reader_status[reader_id] = {
                    ...
                    'timer': None,
                    'stored_values': [],
                    'librarian_id': reader.get('librarian_id', 'N/A')
                }
